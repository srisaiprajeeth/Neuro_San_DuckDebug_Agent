...
[DuckDebug](examples/duckdebug.md) is a rubber-duck debugging companion grounded in 45k labeled Stack Overflow
questions. It scores how clearly you framed a problem using an ensemble of three judges (regex signals, a trained
HQ/LQ classifier, and a kNN vote over labeled neighbours), retrieves how other people framed the same issue,
challenges your assumptions, and reflects the problem back at you. It is an example of MapReduce orchestration
over coded tools that call an MCP server, with an in-process fallback when the server is not running.

**Tags:** `tool`, `MCP`, `MapReduce`, `Kaggle`
